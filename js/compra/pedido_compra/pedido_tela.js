...
const formulario_post = document.getElementById("pedido_compra")
const formulario_item_post = document.getElementById("produto_pedido")
...
$(".fechar_tela_pedido").click(function () {
...
//         $(".title .status_momento_pedido").html("Cotação Iniciada")//alterar a label cabeçalho
//         $(".title .status_momento_pedido").css("display", "")//display block para a label que ira informar o usuario qual é o status momento da venda
//         setTimeout(function () {
//             $(".title .status_momento_pedido").html("Pedido em Andamento..")//alterar a label cabeçalho
//         }, 3000);
...
function IniciarPedido() {
    $("#codigo_nf").val(generateUniqueId())//gerar_codigo_nf automaticamente
    $(".title .status_momento_pedido").html("Pedido iniciado")//alterar a label cabeçalho
    $(".title .status_momento_pedido").css("display", "")//display block para a label que ira informar o usuario qual é o status momento da venda
    setTimeout(function () {
        $(".title .status_momento_pedido").html("Pedido em Andamento..")//alterar a label cabeçalho
    }, 3000);
    ...
    $('#operacao option[value="compra"]').prop('selected', true).trigger('change');
...
if (form_id.value == "") {
    $(".title .sub-title").html("Lançar pedido de compra")//alterar a label cabeçalho
    // var momento_venda = document.getElementById("momento_venda")//status da venda//1-iniciado 2-finalizado 3-edicao
    $(".title .status_momento_pedido").css("display", "none")//display none para a label que ira informar o usuario qual é o status momento da venda

    $('#iniciar_pedido').click(function () {//iniciar venda
        formulario_post.reset(); // redefine os valores do formulário
        IniciarPedido()//conjunto de instruções para iniciar o pedido
    })

} else {//editar venda
    $(".title .sub-title").html("Alterar pedido de compra")//alterar a label cabeçalho
    $(".title .status_momento_pedido").html("Alteração em andamento")//alterar a label cabeçalho
    // show(id_formulario.value) // funcao para retornar os dados para o formulario
    $("#iniciar_pedido").css("display", "none");//inicar venda em none
...
$("#pedido_compra").submit(function (e) {
    e.preventDefault()
    var formulario = $(this);
    if (form_id.value == "") {//cadastrar
        Swal.fire({
            title: 'Tem certeza?',
            text: "Deseja adicionar esse pedido de compra?",
            icon: 'warning',
        ...
        Swal.fire({
            title: 'Tem certeza?',
            text: "Deseja alterar esse pedido de compra? ",
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: 'Não',
            confirmButtonColor: '#3085d6',
...
//formulario para cadastro
$("#produto_pedido").submit(function (e) {
    e.preventDefault()
    var formulario = $(this);
    ...
    if (item_id == '') {
        var retorno = create_produto(formulario, codigo_nf.value)
...
$(".gerar_pdf").click(function () {
    imprimir(codigo_nf.value, 'pdf_pedido')
})
...
$("#cancelar_pedido").click(function () {
    Swal.fire({
        title: 'Tem certeza?',
        text: "Deseja cancelar essa pedido?",
        icon: 'warning',
        showCancelButton: true,
        cancelButtonText: 'Não',
        confirmButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
            cancelar_pedido(form_id.value, '', false)
        }
    })
})
...
function create(dados) {//create dados basicos
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=create&" + dados.serialize(),
    ...
    function sucesso(data) {
        ...
        if ($dados.sucesso == true) {
            ...
            $("#documento").val($dados.response.documento)//retornar o numero de documento
            $("#id").val($dados.response.form_id)//retornar o id 
...
function update(dados) {//create dados basicos
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=update&" + dados.serialize(),
        url: "modal/compra/pedido_compra/gerenciar_pedido.php",
        async: false
    }).then(sucesso, falha);
...
function show(form_id) {
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=show&form_id=" + form_id,
        url: "modal/compra/pedido_compra/gerenciar_pedido.php",
        async: false
    }).then(sucesso, falha);
    ...
    function sucesso(data) {
        ...
        if ($dados.sucesso == true) {
            ...
            $("#data_aprovacao").val($dados.valores['data_aprovacao'])
            $("#status").val($dados.valores['status_id'])
            $("#usuario_id").val($dados.valores['usuario_id'])
            $("#frete").val($dados.valores['frete_id'])
            $("#operacao").val($dados.valores['operacao'])
            $("#desconto").val($dados.valores['valor_desconto'])
            $("#solicitacao").val($dados.valores['numero_solicitacao'])
...
function cancelar_pedido(form_id, usuario_id, check_autorizador) {
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=cancelar_pedido&form_id=" + form_id +
    ...
    function sucesso(data) {
        ...
        if ($dados.sucesso == true) {
            ...
            $.ajax({
                type: 'GET',
                data: "autorizar_acao=true&acao=cancelar_pedido&mensagem=" + $dados.title,
                url: "view/include/autorizacao/autorizar_acao.php",
                success: function (result) {
                    return $(".modal_externo").html(result)
                        + $("#modal_autorizar_acao").modal('show')
                        + $("#autorizar_acao").addClass("autorizar_cancelar_pedido_compra")
...
function show_valores(codigo_nf) {
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=show_valores&codigo_nf=" + codigo_nf,
        url: "modal/compra/pedido_compra/gerenciar_pedido.php",
        async: false
    }).then(sucesso, falha);
    ...
    function sucesso(data) {
        ...
        if ($dados.sucesso == true) {
            ...
            $("#valor_liquido_pedido").val($dados.valores['valor_liquido'])
...
function show_item(form_id) {
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=show_item&form_id=" + form_id,
        url: "modal/compra/pedido_compra/gerenciar_pedido.php",
        async: false
    }).then(sucesso, falha);
...
/*referente ao item */
function create_produto(dados, codigo_nf) {
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=item&" + dados.serialize() + "&codigo_nf=" + codigo_nf,
        url: "modal/compra/pedido_compra/gerenciar_pedido.php",
        async: false
    }).then(sucesso, falha);
...
function tabela_produtos(codigo_nf) {//tabela de produtos
    $.ajax({
        ...
        url: "view/compra/pedido_compra/table/tabela_produtos.php",
        success: function (result) {
...
function destroy_item(item_id, codigo_nf) {
    $.ajax({
        type: "POST",
        data: "formulario_pedido_compra=true&acao=destroy_item&form_id=" + item_id,
        url: "modal/compra/pedido_compra/gerenciar_pedido.php",
        async: false
    }).then(sucesso, falha);
...
function calcular_valor_total() {
    ...
    $('#valor_liquido_pedido').val(valorLiquido);
...
$(document).ready(function () {
    $('.select2-modal').select2({
        dropdownParent: $('#modal_pedido_compra'), // Para garantir que funcione no modal
        width: '100%',
