...
$.ajax({
    ...
    url: "view/estoque/ajuste_preco/table/consultar_produto_promocao.php",
...
$('#salvar_ajuste').click(function () {
    $('#ajuste_preco_promocao').submit();
});

$("#ajuste_preco_promocao").submit(function (e) {
    e.preventDefault()
    var formulario = $(this);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var retorno = ajuste_promocao(formulario, trCount)
        }
    })
})

function ajuste_promocao(dados, qtdRegistros) {

    $.ajax({
        type: "POST",
        data: "formulario_ajuste_preco=true&acao=create_promocao&" + dados.serialize() + "&qt_registro=" + qtdRegistros,
