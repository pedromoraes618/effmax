...
$.ajax({
    ...
    url: "view/estoque/ajuste_estoque1/table/consultar_produto_parametro.php",
...
$('#salvar_ajuste').click(function () {
    $('#ajuste_estoque_parametro').submit();
});

$("#ajuste_estoque_parametro").submit(function (e) {
    e.preventDefault()
    var formulario = $(this);
...
function ajuste_lote(dados, qtdRegistros) {
    ...
    $.ajax({
        type: "POST",
        data: "formulario_ajuste_estoque=true&acao=create_parametro&" + dados.serialize() + "&qt_registro=" + qtdRegistros,
