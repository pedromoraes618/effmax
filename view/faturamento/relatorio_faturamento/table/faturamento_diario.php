<?php
include "../../../../conexao/conexao.php";
include "../../../../modal/faturamento/relatorio_faturamento/gerenciar_faturamento.php";


?>

<div class="card m-1 card-print  card-dashboard position-relative shadow border-0 mb-2 ">
    <div class="card-header header-card-dashboard ">
        <h6><i class="bi bi-exclamation-octagon"></i> Faturamento Diario</h6>
    </div>
    <div class="card-body table-responsive">
        <?php
        if ($qtd_consulta > 0) {
        ?>
            <table class="table  table-hover">
                <thead>
                    <tr>
                        <th scope="col">Data</th>
                        <th scope="col">Qtd Vendas</th>
                        <th scope="col">Valor</th>

                    </tr>
                </thead>
                <tbody>
                    <?php

                    $total = 0;
                    $total_qtdvendas = 0;
                    $qtd = 0;
                    while ($linha = mysqli_fetch_assoc($consulta)) {
                   
                        $qtd += 1;
                        $data = formatDateB($linha['cl_data_movimento']);
                        $qtdvendas = $linha['qtdvendas'];
                        $valor = $linha['valor'];
                        $total_qtdvendas += $qtdvendas;
                        $total += $valor;
                    ?>
                        <tr>

                            <td><?php echo ($data); ?></th>
                            <td><?php echo ($qtdvendas); ?></th>
                            <td><?php echo real_format($valor); ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
                <tfoot>
                    <tr class="table-active">
                        <th scope="col" colspan="1">Total</th>
                        <th scope="col"><?php echo ($total_qtdvendas); ?></th>
                        <th scope="col"><?php echo real_format($total); ?></th>
                    </tr>
                    <tr class="table-active">
                        <th scope="col" colspan="1">Média</th>
                        <th scope="col"><?php echo number_format(($total_qtdvendas/$qtd),0); ?></th>
                        <th scope="col"><?php echo real_format($total/$total_qtdvendas); ?></th>
                    </tr>
                </tfoot>

            </table>
        <?php } else {
            echo '<div class="sem_registro"><img  class="img-fluid img_sem_registro" src="img/financeiro.svg"> </div>';
        } ?>
    </div>
</div>

<div class="card m-1  card-dashboard position-relative shadow border-0 mb-2 ">
    <div class="card-header header-card-dashboard ">
        <h6><i class="bi bi-exclamation-octagon"></i> Detalhado Por Diaria</h6>
    </div>
    <div class="card-body table-responsive">
        <canvas id="detalhado_por_diaria" height="100"></canvas>

    </div>
</div>
<script>
    var media_faturamento_dash_diaria = [
        <?php foreach ($dados_dashboard as $dados) : ?> '<?php echo ($media_valores_dashboard); ?>',
        <?php endforeach; ?>
    ];

    var label_faturamento_dash_diaria = [
        <?php foreach ($dados_dashboard as $dados) : ?> '<?php echo ($dados['label']); ?>',
        <?php endforeach; ?>
    ];
    var valor_faturamento_dash_diaria = [
        <?php foreach ($dados_dashboard as $dados) : ?> <?php echo $dados['valor']; ?>,
        <?php endforeach; ?>
    ];
</script>
<script src="js/faturamento/relatorio_faturamento/table/faturamento_diaria.js"></script>