    ...
    <table class="table table-hover">
        ...
        <tbody id="produtos-tbody">
            ...
            while ($linha = mysqli_fetch_assoc($consultar_produtos)) {
                ...
                if (($data_validade_promocao >= $data_lancamento) and $preco_promocao > 0 and formatDateB($data_validade_promocao) != "") { //promoção
                    $span_promocao = "<span class='text-success'>Promoção " . real_format($preco_promocao) . " Até " . formatDateB($data_validade_promocao) . "</span>";;
                } elseif (($data_validade_promocao < $data_lancamento) and $preco_promocao > 0 and formatDateB($data_validade_promocao) != "") {
                    $span_promocao = "<span class='text-danger'>Promoção de " . real_format($preco_promocao) . " expirada em " . formatDateB($data_validade_promocao) . "</span>";;
                }
