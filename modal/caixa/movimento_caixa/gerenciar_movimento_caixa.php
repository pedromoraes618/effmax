...
if (isset($_GET['movimento_caixa'])) {
    ...
    WHERE 
        ...
        $select = "SELECT lcf.cl_valor_liquido, fpg.cl_descricao as formapg,nf.cl_serie_nf,nf.cl_numero_nf, lcf.cl_data_pagamento,lcf.cl_documento,prc.cl_razao_social
        from tb_lancamento_financeiro as lcf 
        left join tb_parceiros as prc on prc.cl_id = lcf.cl_parceiro_id 
        left join tb_nf_saida as nf on nf.cl_codigo_nf = lcf.cl_codigo_nf 
    ...
    WHERE 
        ...
        $select = "SELECT user.cl_usuario as vendedor, cl_data_movimento,nf.cl_serie_nf,
        nf.cl_numero_nf,cl_razao_social,fpg.cl_descricao as formapg, nf.cl_status_recebimento,
        nf.cl_valor_liquido from tb_nf_saida as nf
        inner join tb_forma_pagamento as fpg on fpg.cl_id = nf.cl_forma_pagamento_id 
        inner join tb_parceiros as prc on prc.cl_id = nf.cl_parceiro_id inner join tb_users as user on user.cl_id = nf.cl_vendedor_id where nf.cl_data_movimento
        between '$data_inicial' and '$data_final' and nf.cl_status_venda = '1' and nf.cl_operacao ='VENDA' order by nf.cl_forma_pagamento_id desc";
        ...
        between '$data_inicial' and '$data_final'  and nf.cl_status_venda = '1'  and nf.cl_operacao ='VENDA'  group by nf.cl_forma_pagamento_id order by nf.cl_forma_pagamento_id desc";
        $consultar_resumo_vendas_fpg = mysqli_query($conecta, $select);

        $select = "SELECT cl_numero_nf_devolucao,cl_data_movimento,nf.cl_serie_nf,
        nf.cl_numero_nf,cl_razao_social,fpg.cl_descricao as formapg, nf.cl_status_recebimento,
        nf.cl_valor_liquido from tb_nf_saida as nf
        inner join tb_forma_pagamento as fpg on fpg.cl_id = nf.cl_forma_pagamento_id 
        inner join tb_parceiros as prc on prc.cl_id = nf.cl_parceiro_id  where nf.cl_data_movimento
        between '$data_inicial' and '$data_final' and nf.cl_status_venda = '1' and (nf.cl_operacao IN ('DEVVENDA', 'ESTORNOVENDA'))  order by nf.cl_forma_pagamento_id desc";
        $consultar_devolucao_Venda_fpg = mysqli_query($conecta, $select);
        $qtd_consultar_devolucao_Venda_fpg = mysqli_num_rows($consultar_devolucao_Venda_fpg);

        $select = "SELECT cl_numero_nf_devolucao,fpg.cl_descricao as formapg,sum(nf.cl_valor_liquido) as valorliquido from tb_nf_saida as nf inner join tb_forma_pagamento as fpg on fpg.cl_id = nf.cl_forma_pagamento_id inner join tb_parceiros as prc on prc.cl_id = nf.cl_parceiro_id where nf.cl_data_movimento
        between '$data_inicial' and '$data_final'  and nf.cl_status_venda = '1'   and (nf.cl_operacao IN ('DEVVENDA', 'ESTORNOVENDA') group by nf.cl_forma_pagamento_id order by nf.cl_forma_pagamento_id desc";
        $consultar_resumo_devolucao_venda_fpg = mysqli_query($conecta, $select);
    } elseif ($acao == "resumo_geral") {
    ...
    WHERE 
        ...
        from tb_lancamento_financeiro as lcf inner join tb_parceiros as prc on prc.cl_id
         = lcf.cl_parceiro_id inner join tb_forma_pagamento as fpg on fpg.cl_id = lcf.cl_forma_pagamento_id where lcf.cl_data_pagamento between '$data_inicial' and '$data_final' 
        and lcf.cl_status_id = '4' and lcf.cl_codigo_nf !='' order by lcf.cl_id desc";
        $consultar_despesa = mysqli_query($conecta, $select);

        $select = "SELECT lcf.cl_valor_liquido, fpg.cl_descricao as formapg, lcf.cl_data_pagamento,lcf.cl_documento,prc.cl_razao_social
        from tb_lancamento_financeiro as lcf inner join tb_parceiros as prc on prc.cl_id
         = lcf.cl_parceiro_id inner join tb_forma_pagamento as fpg on fpg.cl_id = lcf.cl_forma_pagamento_id where lcf.cl_data_pagamento between '$data_inicial' and '$data_final' 
        and lcf.cl_status_id = '4'  and (lcf.cl_codigo_nf = '' or lcf.cl_codigo_nf IS NULL ) order by lcf.cl_id desc";
        $consultar_outras_despesa = mysqli_query($conecta, $select);

        $select = "SELECT lcf.cl_valor_liquido, fpg.cl_descricao as formapg,nf.cl_serie_nf,nf.cl_numero_nf, lcf.cl_data_pagamento,lcf.cl_documento,prc.cl_razao_social
        from tb_lancamento_financeiro as lcf inner join tb_parceiros as prc on prc.cl_id
         = lcf.cl_parceiro_id left join tb_nf_saida as nf on nf.cl_codigo_nf = lcf.cl_codigo_nf inner join tb_forma_pagamento as fpg on fpg.cl_id = lcf.cl_forma_pagamento_id where lcf.cl_data_pagamento between '$data_inicial' and '$data_final' 
        and lcf.cl_status_id = '2' and lcf.cl_codigo_nf !='' order by lcf.cl_id desc";
        $consultar_receita = mysqli_query($conecta, $select);

        $select = "SELECT lcf.cl_valor_liquido, fpg.cl_descricao as formapg, lcf.cl_data_pagamento,lcf.cl_documento,prc.cl_razao_social
        from tb_lancamento_financeiro as lcf inner join tb_parceiros as prc on prc.cl_id
         = lcf.cl_parceiro_id inner join tb_forma_pagamento as fpg on fpg.cl_id = lcf.cl_forma_pagamento_id where lcf.cl_data_pagamento between '$data_inicial' and '$data_final' 
        and lcf.cl_status_id = '2'  and (lcf.cl_codigo_nf = '' or lcf.cl_codigo_nf IS NULL ) order by lcf.cl_id desc";
        $consultar_outras_receita = mysqli_query($conecta, $select);
    }
}
