...
if (isset($_GET['gerar_doc'])) {
    ...
    if ($tipo_ordem == "1" and $tipo == "recibo") {
        $url_qrdcode = $url_init . "/$empresa/view/servico/ordem_servico/documento/recibo.php?gerar_doc=true&tipo=$tipo&codigo_nf=$codigo_nf";
    } elseif ($tipo_ordem == "2" and $tipo == "recibo") {
        $url_qrdcode = $url_init . "/$empresa/view/servico/ordem_servico/documento/recibo_2.php?gerar_doc=true&tipo=$tipo&codigo_nf=$codigo_nf";
    } elseif ($tipo_ordem == "1" and $tipo == "pdf") {
        $url_qrdcode = $url_init . "/$empresa/view/servico/ordem_servico/documento/pdf.php?gerar_doc=true&tipo=$tipo&codigo_nf=$codigo_nf";
    } elseif ($tipo_ordem == "1" and $tipo == "pdf") {
        $url_qrdcode = $url_init . "/$empresa/view/servico/ordem_servico/documento/pdf_2.php?gerar_doc=true&tipo=$tipo&codigo_nf=$codigo_nf";
    } elseif ($tipo_ordem == "2" and $tipo == "pdf_1") { //não inclui valores
        $url_qrdcode = $url_init . "/$empresa/view/servico/ordem_servico/documento/pdf_detalhado_1.php?gerar_doc=true&tipo=$tipo&codigo_nf=$codigo_nf";
    } {
        $url_qrdcode = $url_init . "/$empresa/view/servico/ordem_servico/documento/invalido";
    }
    ...
    $select = "SELECT status.cl_descricao as statusos, cid.cl_nome as cidade, user.cl_nome as atendente, prc.*, os.*,tipos.cl_descricao as tiposervico from tb_os as os 
    left join tb_parceiros as prc on prc.cl_id = os.cl_parceiro_id 
    left join tb_cidades as cid on cid.cl_id = prc.cl_cidade_id 
    left join tb_tipo_servico_os as tipos on tipos.cl_id = os.cl_tipo_servico_id 
   ...
   left join tb_status_os as status on status.cl_id = os.cl_status_id
    ...
    $consultar_os = mysqli_query($conecta, $select);
    if (!$consultar_os) {
        die('Erro no banco de dados ' . mysqli_error($conecta));
    }
    $linha = mysqli_fetch_assoc($consultar_os);
    $os_id = ($linha['cl_id']);
    $razao_social = utf8_encode($linha['cl_razao_social']);
    $cnpj_cpf = formatCNPJCPF($linha['cl_cnpj_cpf']);
    ...
    $condicao_servico = utf8_encode(consulta_tabela($conecta, "tb_parametros", "cl_id", "75", "cl_valor"));
    ...
    $tiposervico = utf8_encode($linha['tiposervico']);
    ...
    $descricao_atividade = utf8_encode($linha['cl_descricao_obra']);
    $local_atividade = utf8_encode($linha['cl_local_obra']);
    $data_prazo_entrega = ($linha['cl_prazo_entrega']);
    $data_entrega_atividade = ($linha['cl_entrega_obra']);
    $valor_fechado = ($linha['cl_valor_fechado']);
    $valor_despesa = ($linha['cl_valor_despesa']);
    $atendente = utf8_encode($linha['atendente']);
    $status_ordem = utf8_encode($linha['statusos']);
    ...
    $valor_bruto = $valor_pecas + $valor_servico;
    $valor_ordem = $valor_pecas + $valor_servico + $valor_despesa - $desconto;

    /*cosultar serviços */
    // $select = "SELECT ios.*,user.cl_usuario from tb_infraestrutura_os as ios left join tb_users as user on user.cl_id = ios.cl_responsavel where
    // ios.cl_codigo_nf ='$codigo_nf' and ios.cl_tipo = 'SERVICO' ";
    // $consulta_servicos = mysqli_query($conecta, $select);
    // if (!$consulta_servicos) {
    //     die("Falha no banco de dados " . mysqli_error($conecta));
    // } else {
    //     $qtd = mysqli_num_rows($consulta_servicos); //quantidade de registros
    // }

    /*cosultar serviços */
    $select = "SELECT ios.*,user.cl_usuario,user.cl_nome from tb_infraestrutura_os as ios left join tb_users as user on user.cl_id = ios.cl_responsavel where
    ios.cl_codigo_nf ='$codigo_nf' and ios.cl_tipo = 'SERVICO' ";
    $consulta_servicos = mysqli_query($conecta, $select);
    if (!$consulta_servicos) {
        die("Falha no banco de dados " . mysqli_error($conecta));
    } else {
        $qtd_servicos = mysqli_num_rows($consulta_servicos); //quantidade de registros
    }


    /*consultar materias */
    $select = "SELECT infr.cl_item_descricao as descservico, infr.cl_id as servid, ios.*,prd.cl_localizacao as localizacao,prd.cl_estoque as estoque from 
    tb_infraestrutura_os as ios 
    left join tb_infraestrutura_os as infr on infr.cl_id = ios.cl_servico_destinado_id 
    left join tb_produtos as prd on prd.cl_id = ios.cl_produto_id 
    where ios.cl_codigo_nf ='$codigo_nf' and ios.cl_tipo = 'MATERIAL' order by ios.cl_id,ios.cl_servico_destinado_id ";
    $consulta_material = mysqli_query($conecta, $select);
    if (!$consulta_material) {
        die("Falha no banco de dados " . mysqli_error($conecta));
    } else {
        $qtd_material = mysqli_num_rows($consulta_material); //quantidade de registros
    }



    /*consultar despesas que contibilizam */
    $select = " SELECT lcf.cl_descricao as descricao, lcf.*,clf.*,clf.cl_descricao as classificacao FROM tb_despesa_os as desp 
                    left join tb_lancamento_financeiro as lcf on lcf.cl_id = desp.cl_lancamento_id
                    left join tb_classificacao_financeiro as clf on clf.cl_id = lcf.cl_classificacao_id
                     where desp.cl_codigo_nf ='$codigo_nf' and desp.cl_tipo= '1' and lcf.cl_status_id ='4' ";
    $consulta_despesa = mysqli_query($conecta, $select);
    if (!$consulta_despesa) {
        die("Falha no banco de dados " . mysqli_error($conecta));
    } else {
        $qtd_despesa = mysqli_num_rows($consulta_despesa); //quantidade de registros
    }
    /* equipamento */
}

if (isset($_GET['despesa_ordem_servico'])) {
    include "../../../../conexao/conexao.php";
    include "../../../../funcao/funcao.php";
    ...
    if (isset($_GET['form_id'])) {
        ...
        $select = "SELECT desp.*,fpg.cl_descricao as fpagamento,lcf.*,clf.cl_descricao 
        as classificacao,sts.cl_descricao as statusl,lcf.cl_status_id as statusid from tb_despesa_os as desp 
         inner join tb_lancamento_financeiro as lcf on lcf.cl_id = desp.cl_lancamento_id 
         inner join tb_forma_pagamento as fpg on fpg.cl_id = lcf.cl_forma_pagamento_id 
         inner join tb_classificacao_financeiro as clf on clf.cl_id = lcf.cl_classificacao_id 
         inner join tb_status_recebimento as sts on sts.cl_id = lcf.cl_status_id
         where desp.cl_codigo_nf ='$codigo_nf' and desp.cl_tipo = '1' order by lcf.cl_data_pagamento,lcf.cl_status_id ";
        $consulta_despesa_contabiliza = mysqli_query($conecta, $select);
        if (!$consulta_despesa_contabiliza) {
            die("Falha no banco de dados " . mysqli_error($conecta));
        } else {
            $qtd = mysqli_num_rows($consulta_despesa_contabiliza); //quantidade de registros
        }

        $select = "SELECT desp.*,fpg.cl_descricao as fpagamento,lcf.*,
        clf.cl_descricao as classificacao,sts.cl_descricao as statusl,lcf.cl_status_id as statusid from tb_despesa_os as desp 
        inner join tb_lancamento_financeiro as lcf on lcf.cl_id = desp.cl_lancamento_id 
        inner join tb_forma_pagamento as fpg on fpg.cl_id = lcf.cl_forma_pagamento_id 
        inner join tb_classificacao_financeiro as clf on clf.cl_id = lcf.cl_classificacao_id 
        inner join tb_status_recebimento as sts on sts.cl_id = lcf.cl_status_id
        where desp.cl_codigo_nf ='$codigo_nf' and desp.cl_tipo = '2'  order by lcf.cl_data_pagamento,lcf.cl_status_id ";
        $consulta_despesa_nao_contabiliza = mysqli_query($conecta, $select);
        if (!$consulta_despesa_nao_contabiliza) {
            die("Falha no banco de dados " . mysqli_error($conecta));
        } else {
            $qtd = mysqli_num_rows($consulta_despesa_nao_contabiliza); //quantidade de registros
        }
    } else {
        $form_id = null;
    }
...
if (isset($_GET['registro_ordem_servico'])) {
    include "../../../../conexao/conexao.php";
    include "../../../../funcao/funcao.php";
    $usuario_id = verifica_sessao_usuario();
    ...
    if (isset($_GET['form_id'])) {
        $form_id = $_GET['form_id'];
        $codigo_nf = consulta_tabela($conecta, "tb_os", "cl_id", $form_id, "cl_codigo_nf");
        $select = "SELECT lcf.*,fpg.cl_descricao as fpagamento,sts.cl_descricao as statusl,sts.cl_id as idstatusl FROM tb_lancamento_financeiro as lcf inner join tb_forma_pagamento as fpg
         on lcf.cl_forma_pagamento_id = fpg.cl_id 
          inner join tb_status_recebimento as sts on sts.cl_id = lcf.cl_status_id
           where cl_codigo_nf ='$codigo_nf' and lcf.cl_status_id in(2,5) and lcf.cl_tipo_lancamento='RECEITA' order by lcf.cl_status_id asc ";
        $consulta_financeiro = mysqli_query($conecta, $select);
        if (!$consulta_financeiro) {
            die("Falha no banco de dados " . mysqli_error($conecta));
        } else {
            $qtd = mysqli_num_rows($consulta_financeiro); //quantidade de registros
        }
    } else {
        $form_id = null;
    }
...
if (isset($_GET['material_ordem_servico'])) { //consultar materias da ordem de servico
    include "../../../../conexao/conexao.php";
    include "../../../../funcao/funcao.php";
    $usuario_id = verifica_sessao_usuario();

    $form_id = isset($_GET['form_id']) ? $_GET['form_id'] : 0;
    $tipo_produto_id = isset($_GET['tipo_produto_id']) ? $_GET['tipo_produto_id'] : '';

    $codigo_nf = consulta_tabela($conecta, "tb_os", "cl_id", $form_id, "cl_codigo_nf");
    $select = "SELECT  infr.cl_item_descricao as descservico,  infr.cl_id as servid,
         ios.*,prd.cl_localizacao as localizacao,prd.cl_estoque as estoque from tb_infraestrutura_os as ios
        left join tb_infraestrutura_os as infr on infr.cl_id = ios.cl_servico_destinado_id 
        left join tb_produtos as prd on prd.cl_id = ios.cl_produto_id 
        where ios.cl_codigo_nf ='$codigo_nf'";
    if ($tipo_produto_id == "10") {
        $select .= " and ios.cl_tipo = 'MATERIALMOBILIZADO' ";
    } else {
        $select .= " and ios.cl_tipo = 'MATERIAL'";
    }
    $select .= " order by ios.cl_servico_destinado_id  desc ";
    ...
    if (!$consulta_material) {
        ...
        $qtd = mysqli_num_rows($consulta_material); //quantidade de registros
    }
}

if (isset($_GET['servico_1_ordem_servico'])) { //consultar SERVICOS da ordem de servico
    include "../../../../conexao/conexao.php";
    include "../../../../funcao/funcao.php";
    $usuario_id = verifica_sessao_usuario();
    ...
    $form_id = isset($_GET['form_id']) ? $_GET['form_id'] : 0;
    $codigo_nf = consulta_tabela($conecta, "tb_os", "cl_id", $form_id, "cl_codigo_nf");
    $select = "SELECT prc.*, ios.*,user.cl_usuario from tb_infraestrutura_os as ios
        left join tb_users as user on user.cl_id = ios.cl_responsavel 
        left join tb_parceiros  as prc on prc.cl_id = ios.cl_parceiro_terceirizado_id
         where ios.cl_codigo_nf ='$codigo_nf' and ios.cl_tipo = 'SERVICO' ";
    $consulta_servicos = mysqli_query($conecta, $select);
    if (!$consulta_servicos) {
        die("Falha no banco de dados " . mysqli_error($conecta));
    } else {
...
if (isset($_GET['equipe_ordem_servico'])) { //consultar SERVICOS da ordem de servico
    include "../../../../conexao/conexao.php";
    include "../../../../funcao/funcao.php";

    $form_id = isset($_GET['form_id']) ? $_GET['form_id'] : 0;
    $select = "SELECT * from tb_equipe_servico where cl_servico_id = '$form_id' ";
    $consulta_equipe_servico = mysqli_query($conecta, $select);
    if (!$consulta_equipe_servico) {
        die("Falha no banco de dados " . mysqli_error($conecta));
    } else {
        $qtd = mysqli_num_rows($consulta_equipe_servico); //quantidade de registros
    }
}


if (isset($_GET['resumo_valores']) or isset($_GET['adicionar_servicos']) or isset($_GET['ordem_servico']) or isset($_GET['diagnostico_tecnico'])  or isset($_GET['adicionar_taxa']) or isset($_GET['adicionar_despesa'])  or isset($_GET['adicionar_pecas']) or isset($_GET['faturar_os'])) {
    include "../../../conexao/conexao.php";
    include "../../../funcao/funcao.php";
    $usuario_id = verifica_sessao_usuario();
    $tipo_ordem = consulta_tabela($conecta, "tb_parametros", "cl_id", 66, "cl_valor"); //1-peças 2 -obra
...
if (isset($_GET['consultar_ordem_servico'])) {
    include "../../../../conexao/conexao.php";
    include "../../../../funcao/funcao.php";
    $tipo_ordem = consulta_tabela($conecta, "tb_parametros", "cl_id", 66, "cl_valor"); //1-peças 2 -obra

    $consulta = $_GET['consultar_ordem_servico'];
    $data_inicial = $_GET['data_inicial'];
    $data_final = $_GET['data_final'];
    $data_inicial = ($data_inicial . ' 01:01:01');
    $data_final = ($data_final . ' 23:59:59');

    $os_id = isset($_GET['form_id']) ? $_GET['form_id'] : '';
    if (!empty($os_id)) {
    ...
    if ($consulta == "inicial") {
        ...
        $select = "SELECT os.*,prc.cl_razao_social,prc.cl_nome_fantasia,user.cl_usuario,tpo.cl_descricao as 
        tiposervico,status.cl_descricao as statusos from tb_os as os 
       inner join tb_parceiros as prc on prc.cl_id = os.cl_parceiro_id inner join tb_users as user on user.cl_id = os.cl_atendente_id 
       inner join tb_tipo_servico_os as tpo on tpo.cl_id = os.cl_tipo_servico_id inner join tb_status_os as status on status.cl_id = os.cl_status_id
        ...
        if (!empty($parceiro_id)) {
            $select .= " and os.cl_parceiro_id = '$parceiro_id' and os.cl_id !='$os_id' and  os.cl_situacao= 0 ";
        }
        $select .= " order by os.cl_data_abertura desc ";
        ...
        $select = "SELECT os.*,prc.cl_razao_social,prc.cl_nome_fantasia,user.cl_usuario,tpo.cl_descricao as tiposervico,status.cl_descricao as statusos  from tb_os as os 
       inner join tb_parceiros as prc on prc.cl_id = os.cl_parceiro_id inner join tb_users as user on user.cl_id = os.cl_atendente_id
       inner join tb_tipo_servico_os as tpo on tpo.cl_id = os.cl_tipo_servico_id inner join tb_status_os as status on status.cl_id = os.cl_status_id
        WHERE os.cl_data_abertura between '$data_inicial' and '$data_final' and ( prc.cl_razao_social  like '%$pesquisa%'
       or prc.cl_nome_fantasia  like '%$pesquisa%' or prc.cl_cnpj_cpf like '%$pesquisa%'  or os.cl_numero_serie like '%$pesquisa%' or os.cl_numero_nf like '%$pesquisa%' or os.cl_equipamento like '%$pesquisa%' ) ";
            ...
            if ($form_id == "") {
                ...
                if ($operacao_update and $update_valor_servico and $update_valor_liquido) {
                    ...
                    if (isset($_POST['nome_equipe_1'])) { //Adicionar os menbros da equipe 
                        ...
                        for ($i = 1; $i <= 15; $i++) {
                            if (isset($_POST["nome_equipe_$i"])) {
                                $nome_equipe = utf8_decode($_POST["nome_equipe_$i"]);
                                $matricula_equipe = utf8_decode($_POST["matricula_equipe_$i"]);
                                $funcao_equipe = utf8_decode($_POST["funcao_equipe_$i"]);
                                $data_inicio_equipe = $_POST["data_inicio_equipe_$i"];
                                $data_final_equipe = $_POST["data_final_equipe_$i"];
                                $codigo_nf_equipe = $codigo_nf . "_" . $i;
